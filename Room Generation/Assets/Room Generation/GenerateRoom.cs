...
namespace MMRoomGeneration
{
    ...
    {
        [Title("Random Seed", "Changing this will create a new room.")]
        public int Seed = 0;
        System.Random RandomSeed;


        [Title("Chance that an Encounter Island will be Enemy (high) instead of a Resource.")]
        [Range(0f, 1f)]
        public float EncounterWillBeEnemyOrResource = 0.5f;

        [Title("Sprite Shapes to set the biome theme.")]
        ...
        [Title("Doors", "Tick to have a door generated")]
        ...
        {
            foreach (IslandPiece i in Pieces)
                i.InitIsland(RandomSeed, SpriteShapeSecondary);
        }
        ...
        void CollateLists()
        {
            ...
            foreach (IslandPiece i in ResourcePieces)
            {
                if (i.GetConnectorsDirection(IslandConnector.Direction.North, false).Count > 0) NorthIslandResourcesPieces.Add(i);
                if (i.GetConnectorsDirection(IslandConnector.Direction.East, false).Count > 0) EastIslandResourcesPieces.Add(i);
                if (i.GetConnectorsDirection(IslandConnector.Direction.South, false).Count > 0) SouthIslandResourcesPieces.Add(i);
                if (i.GetConnectorsDirection(IslandConnector.Direction.West, false).Count > 0) WestIslandResourcesPieces.Add(i);
            }
        ...
        IslandPiece GetRandomEncounterIsland()
        {
            if (RandomSeed.NextDouble() < EncounterWillBeEnemyOrResource)
            {
                return StartPieces[RandomSeed.Next(0, StartPieces.Count)];
            }
            else
            {
                return ResourcePieces[RandomSeed.Next(0, ResourcePieces.Count)];
        ...
        IslandPiece GetResourceIslandListByDirection(IslandConnector.Direction Direction)
        {
            switch (Direction)
            {
                case IslandConnector.Direction.North: return NorthIslandResourcesPieces[RandomSeed.Next(0, NorthIslandResourcesPieces.Count)];
                case IslandConnector.Direction.East: return EastIslandResourcesPieces[RandomSeed.Next(0, EastIslandResourcesPieces.Count)];
                case IslandConnector.Direction.South: return SouthIslandResourcesPieces[RandomSeed.Next(0, SouthIslandResourcesPieces.Count)];
                case IslandConnector.Direction.West: return WestIslandResourcesPieces[RandomSeed.Next(0, WestIslandResourcesPieces.Count)];
            }
            return null;
        }
        ...
        void CreateStartPiece()
        {

            StartPiece = Instantiate(GetRandomEncounterIsland(), Vector3.zero, Quaternion.identity, RoomTransform.transform);
        ...
        void GeneratePath(RoomPath Path)
        {
            ...
            if (true)//RandomSeed.Next(0, 3) <= 0)
            {
                ...
                if (ClosestConnector != null)
                    CurrentPiece = ClosestConnector.GetComponentInParent<IslandPiece>();
